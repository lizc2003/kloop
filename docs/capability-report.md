...
| 差距项 | 收敛 | 补齐路径 | 触发条件 |
|---|---|---|---|
| 压缩后重注入最近读过的 ≤5 文件 | cc + chord | **✅ plan 203(2026-09-24,`97884e4`)**:压缩时经真实 `read_file` 重读折叠段里最近读过的 ≤5 个文件(单文件 12000 字符 / 总 40000 / ≤ 剩余窗口 1/4),作为 `RestoredFiles` 写进替换结果并持久化——与 chord 每次请求临时注入相反,为的是不击穿 prompt cache;过时由 plan 197 的变更提醒接手。同一 plan 还让用户原话由运行时逐字跨代继承(`UserAnchors`),不经摘要模型 | 已完成 |
