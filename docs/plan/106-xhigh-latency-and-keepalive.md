...
- **不是重复读同样的东西。** 同一个大文件读了 60 次,其中 **56 个不同的
...
- **子 agent 结论一节**(两家都没有,kloop 特有)。动机是片 16 实测的 99 个请求里 83 个
  在主 agent 重做子 agent 已审过的文件——**但见片 21:这条动机后来未能被实验证实。**
...
## 片 21 ✅ — 提示词能否减少压缩后重读:未能证实

片 19 的六条里,五条是防错(禁工具、防伪造用户消息、安全约束逐字、草稿块、转录指针),
...
压缩次数是 5 vs 11,而重读率与压缩次数强相关(每压一次多一次失忆机会)。**自变量的结果
被另一个随机量混杂了**,不可比。

改成**按压缩事件归一**:固定看每次压缩后的前 10 个带文件目标的工具调用,数其中有多少
命中该次压缩之前已见过的目标。这样压缩次数不同也能比,而且是纯重新分析,不花钱。

| | 有效边界 | 每边界重读率 | 均值 |
| --- | --- | --- | --- |
| 旧提示词(第 8/9 轮 + A 组) | 10 | 100 90 80 100 100 90 70 80 80 70 | 86% |
| 新提示词(B 组) | 9 | 100 90 100 80 80 60 50 70 80 | 79% |

方向反过来了,但**判不了**:7 个百分点的差,而组内跨度是 50–100;只比同配置的 A vs B
是 75% vs 79%(新的还略差),4 个边界对 9 个边界。这个噪声水平下要分辨 5–10 个百分点,
需要的样本量远超一次会话能跑的量。

**结论:假设未能证实。**"子 agent 结论"和"别重新推导"两条保留,但不再声称它们减少重读。

顺带看清一件事:**重读率普遍在 70–100%,新旧一致。**压缩之后模型几乎必然重读已见过的
文件,这不像措辞能治,更像压缩本身的信息损失决定的——摘要里再怎么写"别重做",具体到
某一行代码时,摘要就是没有那个信息。真要治,方向在别处,而且这三条都已经做了:

- **少压缩**:片 18 把窗口抬到真实值,第 10/12 轮压缩 0 次,是本 plan 唯一被复现的有效改动。
- **压缩时多留原文**:`KEEP_RECENT_TOKENS` 2k → 20k(片 15)。
- **让重读变便宜**:转录指针(片 19)+ 读取上限 7k → 30k(片 15)。


## 片 22 ✅ — "已产出内容不可能丢"照参考项目做到什么程度

用户问 cc 那句"内容在循环里就 yield 了,不可能丢"能不能照做。查证之后答案是:
**生成器这条路走不了,而且走了也买不到想要的东西。**

- 走不了:`rust-toolchain.toml` 钉在 1.96.1 稳定版,`async gen` 块不可用;稳定 Rust 的
  等价物是 channel(kloop 的 provider 层已经是这个形状)。
- 买不到:`AgentTool.tsx:1086` 是 `for await (const msg of runAgent(...))` ——cc 也要
  **消费**子 agent 的流,再在 1399 行 `agentMessages.findLast(...)` 挑出最后一条变成工具
  结果。**子 agent 边界上流终究塌缩成一个值,cc 也一样。**

cc 真正的 robustness 不来自 `yield`,来自**累积发生在消费循环里,退出路径只读那个已经填好
的变量**(`agentMessages` 在 `break` 之前就装满了)。这与片 11 加的 `produced_text`
累积器同构。kloop 早就有 cc 那两条通路:UI 每轮 `ui.emit(ItemDelta)`,history 每轮
`record_provider_assistant`。**唯一会丢的是 `final_text`——交还给调用方的那个字符串。**

### 缺的不是架构,是覆盖率

`run_turn` 有 22 个出口。片 11 只修了被实测撞到的 `MaxRounds` 一处;枚举之后发现循环内
还有 **11 处**在已产出内容之后返回 `String::new()`:轮间重建工具失败、压缩中被取消、
溢出且压缩不可用、反应式压缩无变化、反应式压缩失败、provider 终态失败、fallback 也失败、
结构化输出错误、工具结果后被取消,以及 **`Completed` 且末轮只有工具调用没有文本**——
子 agent 干完活但末轮是纯工具,父 agent 同样拿到空串。

先写三条枚举测试(终态失败 / 溢出无压缩 / 反应式压缩失败),**三条全红、全是空串**,证实
漏是真的而不是读错代码;再把循环内所有 `final_text: String::new()` 机械替换为
`produced_text.clone()`(12 处),三条转绿,全量 0 失败。循环**之前**的三处保持空串——
那时确实什么都没产出。

这不是编译期强制(Rust 没法阻止有人再写一次 `String::new()`),但把规则收敛成一句可检查
的话:**循环内的出口永远不写 `String::new()`**,加上三条枚举测试兜底。

**后续**:用户要求把类型级保证也做掉,见 **Plan 108**——22 个出口收成单一出口,循环内
只能 `break 'turn Ending { text: Option<String> }`,`None` 即"交回已产出",三条枚举测试
当验收。
