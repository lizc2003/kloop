...
三家都命中、且回源码核实为真的那条最重的缺陷是一次**错误分类退化**：删掉一层
selector 之后，"调用方给了个不存在的名字"这种 400 类错误退化成了 503 服务端错误，
还带静态文案、并被记成服务端故障。后果是调用方拼错名字会拿到"瞬时故障"，SDK 一直重试。
