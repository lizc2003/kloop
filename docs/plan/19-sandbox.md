# Plan 19 — 沙箱基建 ✅(片 1 完成 68bcf18;片 2 完成 41b4fd1;片 3 完成 768905d)
...
**剩余(仅平台扩展)**:① Linux 片对着 bwrap+seccomp 设计(landlock 是 legacy);`sandbox/` 已是目录模块,平台后端落 sibling 文件(`linux.rs`)——**不拆独立 crate**(Linux-only 依赖 `cfg(target_os)` 门控、helper 二进制 arg0 dispatch 复用主二进制;拆 crate 只在 unsafe 面/编译成本有痛感时再议)。本机无 Linux,宜等仓库推远端后 Linux CI 能实跑再做。② Windows 更后,且缝要重塑:当前"沙箱 = argv 前缀"是 macOS/Linux 形,Windows(restricted token / AppContainer)无包装二进制,缝要改成"沙箱负责 spawn"的 trait(macOS/Linux 用前缀实现)。③ 小挂账:seatbelt 下 `.git` 文件型(worktree gitdir 指针)未解析真实 gitdir(codex 有,罕见场景);bash 工具描述提沙箱但 system prompt 未提(靠 denial hint / escalation 事发时教学,验收显示够用)。macOS 侧沙箱(执行原语 + auto-allow + escalation 环)已完整。
