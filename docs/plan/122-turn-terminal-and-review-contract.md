...
被改的那个适配器里，金额字段只在定价表存在时才设，credits 字段却在这个判断之外
无条件设；而定价表为空是真实部署形态（同模块另一处注释写明了这一点）。照这条建议
...
kloop 只给了「人为配一个零折扣价格行」这一条。而那个格式化函数对非正数直接返回
`"0"`、校验又接受 0，**上游直接报 `credits: 0` 就能触发**，不需要任何异常配置——
被改的那份 API 文档刚好承诺了「明确返回 0 时仍保留」，缺金额字段在这条路径上最打脸。
...
1. **正数下溢被当成合法零结算。** 被改模块自带的格式化函数对正数直接按定点小数
   输出，`4e-09` 变成 `"0.00000000"`;而仓库对这条规则有**两个**明确 owner，
   都判「正数但格式化成零」为不可表示——**只有那个模块自己的格式化函数绕开了**。
   实测症状是上游金额非零、结算金额为零、且无告警,**静默漏收**。
   codex 把处方开在注入层（不该拿 credits 当零金额的证明）,claude
   复核后指出那一步不成立——下溢时账本本身就是 0,wire 上的 `cost: 0` 是忠实
   报账,**错的是账本不是 wire**,于是修在源头。
2. **文档过度承诺。** 那次提交自己新写的一份 API 文档说"`cost`
...
- **第 1 条**:kloop 第二轮读了定价模块的三个文件——它已经站在"精度契约"这条
  线上,**却没有回到被改的那个模块去看它有没有绕开这条契约**。它两轮都没打开
  那个模块里的格式化函数(第一轮引用过同一个文件的另一处讲 discount 合法性,
  没看紧邻的格式化函数)。
- **第 2 条**:那次提交的改动清单里有一份 API 文档,kloop 第二轮读的是同目录
  的另一份,**被这次提交改动的那份文档从未被打开**,
  排除列表里也没有任何一条与文档有关。
...
不可表示」并进了 fail-closed 分支,而领域层的契约写着这种输出必须保持
**deliverable**;于是已完成的任务被丢弃,编排层还会把无 response 的普通 error
改判成可重试,去重跑一次已经完成的任务。
