...
4. **判据是"这个项目的私有目录在不在"**(用户 2026-09-22 定)。`~/.kloop/projects/v1/<id>/`
   只有 kloop 自己会建,它存在就意味着"这台机器的主人已经在这儿跑过了"——里面装的是信任
   记录、转录还是持久授权都一样。于是不必单独去查会话历史:早于这道门的老项目本来就有
   目录,自然不问。`trust.json` 的**主要作用就是把这个目录建出来**,内容是留给以后打开它
   的人看的。撤销 = 删目录。
...
```json
{ "version": 1, "projectId": "p1_…", "grantedAt": "2026-09-22T07:46:15Z" }
```

**这个文件是只写的**:没有任何代码读它回来,判据是它所在的目录。所以也**没有 `trusted`
字段**——一个恒为 true 的布尔只会给人"改成 false 试试"的错觉,而那条路不存在。`grantedAt`
同理,是给以后打开它的人看的("这条授权什么时候给的"),形式取 `utc_timestamp`(可读的
`YYYY-MM-DDTHH:MM:SSZ`),不用 rollout 那种毫秒 `ts`——那是给只有 kloop 读的账本用的。
