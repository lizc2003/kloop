...
> 历史记录。
>
> **⚠️ 2026-09-22 整体撤销**:`.github/` 已删除,用户判「目前不需要 CI」。推远端后
> 这条流水线跑过 9 次、9 次全红,全红在 `clippy -D warnings`——matrix 取 `@stable`
> (已到 1.98.1)而仓库 pin 1.96.1,红的是工具链版本差、不是代码。理由与取证见
> HANDOFF 教训 175;现在 `make check` 是唯一的门。
