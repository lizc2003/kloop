...
**前置**:第五节的建议是"先别做,等一个具体痛感"。若用户拍板要做,按下面验收。

1. `find_definition` 在 Rust、TS/JS、Python 三种语言上各有测试,且覆盖一个**同名符号**
   场景(两个文件里同名函数,返回两条而不是猜一条)。
2. **冷启动不阻塞**:索引未建好时工具立刻返回"正在建索引",不是卡住等全仓解析完。
   有测试锁住这条。
3. **索引不落工作区**:建完索引后 `git status --porcelain` 为空。
4. `target/`、`node_modules/`、`.git/` 被排除,且走的是 `search.rs` 已有的 .gitignore
   逻辑而不是新写一套。
5. 增量生效:改一个文件后只重解析那一个,有测试断言其余文件未被重解析。
6. **编译代价有数**:记一次 grammar 加入前后的 `cargo build` 耗时与二进制体积进 ✅ 节。
   涨得离谱就减语言,别硬上。
7. 仓库完成标准照旧(fmt / clippy -D warnings / test,各自取退出码)。
