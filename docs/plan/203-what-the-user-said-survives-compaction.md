...
✅ 2026-09-24,`97884e4`(两片一次提交)。

- **第五节的问题**:收窄(用户「同意」)。第 2 节改成"意图怎么变、为什么变",并写明原话由运行时逐字保留;
  提示词测试钉住了 `kept verbatim by the runtime` 这一句。
- **2.2 开工时定的点**:解析上一代 `UserAnchors` 的固定格式,不挂结构化字段。每行正文前缩进两格,
  于是正文里的空行、`- ` 开头的行、甚至两个标签原文都不会截断条目;格式对不上就当作没有上一代。
  往返测试(写出 → 解析 → 再写出逐字节相同)覆盖了这些边角。
- **偏离 plan 的一处:多加了 `Injected::Hook` 与 `Injected::Harness`**。plan 说"hook 输出不算用户的话",
  可开工一查,hook stdout、todo/变更文件提醒、截断续写/流恢复/结构化输出的 nudge **全都是无标记的
  `Message::user_text`**——不打标记,"谁算用户说的"就只能靠猜文案。补上标记之后 TUI 重放也跟着对了
  (之前这些会显示成用户输入,正是 plan 116 那类错)。`/context` 里它们从"user text"挪到"injected text"。
- **片 2 的三个落地细节**:
  - 预算里的"剩余窗口"= `effective_window` − 替换结果各消息的估算 token(不含 system 与工具定义,
    偏宽一点,但 40000 字符那道硬上限兜着);没配窗口时只有字符上限。
  - **每个文件在读之前先按磁盘内容预估大小**,放不下就停,不先读再丢——读一次会把那些行登记成
    "在上下文里",丢掉它们会让重读提示对模型撒谎。`forget_context_reads` 因此挪到这些读**之前**。
  - 走真实 dispatch,所以显式的 `ask` 规则会在压缩时弹确认(与"ask 从不记住"一致);deny/敏感路径
    在读之前就用 `read_path_blocked` 跳过,不会弹。
- **验证**:`make check` 全绿。新测试在 `compact/anchors.rs`(格式往返、谁算用户说的、预算与截断)、
  `compact/restore.rs`(截断续读行、总量与 1/4 窗口两道上限、notebook 与尾巴已读跳过)、
  `compact/plan203_acceptance_tests.rs`(首次压缩整对象断言、连压三次原始请求逐字节不变且摘要只有一句话、
  开头压缩产物被越过、7 个文件里放回最近 5 个且跳过尾巴已读/已删/被 deny、放回后可直接 edit、
  外部修改被 plan 197 提醒、rollout 重载逐字节相同)。
- **`refs/chord` 未退休**:plan 204 还没做,由它负责。
