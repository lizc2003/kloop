...
4. resume 恢复最后 committed route 和 remembered models；fork/原地 rewind 按所选 canonical rollout boundary 恢复当时 route，而不是 process 当前默认或源 thread 最新 route。**（本条已被 Plan 132 §六 推翻：重新打开一个会话——resume/fork——一律用当前配置默认 route，`/provider` 只活到本次会话结束；只有进程内 rewind 继续沿用会话当前 route。）**route transition 必须参与 cut/recovery 顺序，不能只按 message count 猜前后。
