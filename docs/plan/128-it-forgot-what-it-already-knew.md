...
「某个包的 `go test` 因沙箱禁止 `httptest` 绑定 IPv6 端口而失败」——
...
前者 34/39 是**探索**动作，而且读的区间与压缩前大量重叠（同一个大文件
...
codex 轮2：wc -l <几份文档>                       ← 先探大小
      轮3：git diff … -- <文件 A> <文件 B> …       ← 按文件分批
      轮4：git diff … -- <文件 C> …                ← 继续分批
```
...
**两个数字都由 `scripts/review-pace.py` 算出**（不带参数取最近改动的 rollout，可选第二
个参数按任务关键字过滤）；它在本片的样本上复现过下面这些基线，输出逐位相符。

两个数字，分别对应 4.0 + 4.1 和 4.2：
