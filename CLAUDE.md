...
- **kloop 是主开发对象**。`refs/codex`(引擎 codex-rs)、`~/work/claude-code`、`refs/claw-code`、`refs/codewhale` 仅供参考,绝不在其上开发或推送;四个参考库的导读与调研结论在 `refs/README.md`。`refs/claw-code` 与 `refs/codewhale` 是被 `.gitignore` 排除的本地只读克隆。kloop 自身开发直接在 main 分支,不开 feature 分支。**例外(2026-07-20 拍板)**:目标是让 kloop 引擎替换 codex 的 旧引擎二进制 二进制,为此 `~/work/桌面前端仓库/app`(Tauri 前端)可**在专门分支上**改造,适配 kloop 自己设计的原生 agent 协议(不再逐字节兼容 codex 的 app-server wire)。
