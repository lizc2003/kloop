...
- **验证**:`cd rust && cargo test`(1 秒,无网络无 key);`cargo run -p kloop -- --mock`。真实 API 的代理和 key 问用户要,勿写进任何提交文件。
