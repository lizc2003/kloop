...
pub fn tool_defs(search_backend: Option<&str>) -> Vec<ToolDef> {
    let mut defs = vec![ToolDef {
        name: WEB_FETCH.into(),
        description: "Fetch a URL and return its bounded plain-text content (HTML is converted, tags stripped). HTTP is upgraded to HTTPS. Same-site redirects are followed; a cross-host redirect is reported back so you can fetch the new URL explicitly. Refuses private/internal addresses and embedded credentials. This tool returns page content directly; it does not run a second model prompt over the page. The whole body is returned — there is no text cap — so when first fetching a large target (a spec, an OpenAPI document, a full page dump), call this from inside run_program: there the body stays in a JS variable and never enters the context, and you return only what you extracted. If a body you already fetched was offloaded, do not fetch it again — reuse it with tools.read_offloaded inside the program, which its pointer spells out.".into(),
        schema: json!({
            "type": "object",
            "properties": {
                "url": {
