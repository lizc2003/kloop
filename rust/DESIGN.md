...
- **Predictive**: before each sampling round, estimate the next request plus
  ...
  A predictive failure is not fatal — the round samples anyway and may still
  overflow into the reactive path — but it is not free either: the summary
  request carries nearly the whole history, the most expensive request of the
  round, and the next turn is still over the line and would pay for it again.
  So two consecutive failures (a user cancel is not one) pause predictive
  compaction for the rest of that turn and the next three turns, with one note
  saying so. Counted in turns, not rounds: one turn can run dozens of rounds, and a
  three-round pause would be over before it saved anything. Any applied compaction — predictive, reactive
  or `/compact`, all landing in `History::replace_all` — closes the breaker and
  clears the count, and so does a pause running out. The breaker is only
  consulted by the predictive path: reactive compaction is the turn's last way
...
starts with its recent conversation visible instead of a blank screen. What it
...
- **Instruction files**: one name per directory, `AGENTS.md`. Layers, in order:
  ...
  truncated and the rest skipped, with startup warnings. `AGENTS.md` is the only
  name read; a reader that wants a different filename points at it with `@`.
...
why the switch was added) end the report as a question carrying the verdict each
answer would produce; and a review runs the affected package's tests, never the
project's pre-merge gate. It runs **inline**, in the conversation that asked for
it: a forked review keeps its evidence in the sub-agent, so the delegating agent
— which must not take a sub-agent's report as fact — is left re-collecting it
(measured: 42 minutes and 254 sub-agent tool calls, then 67 more in the parent
plus a second reviewer).
They come last in precedence: writing `.kloop/skills/code-review/SKILL.md` replaces the
...
`--help`, `-V`/`--version`, and `--list-sessions` are local fast paths: they
return before provider credential resolution, runtime loading, and MCP
connection. `--version` prints the same stamp the banner carries
(`kloop v0.1.0 (2319ea3)`, plan 161), so "which build is this" is answerable
without starting a session; `-v` is deliberately not an alias, leaving the
lowercase letter for a future verbose flag. They remain usable with a
...
#
# No provider key speaks about that field at all: it is derived from the rail, the
# model and the effort, so nothing in a profile can contradict the effort asked
# for. A gateway that cannot take the field would 400, loudly and with its
# endpoint named; a key for that case will be added when one turns up, not before.
#
